      ...
      <div class="container-wrapper">
        ...
        <ion-modal :is-open="isOpen" :backdrop-dismiss="false">
          <div class="modal-wrapper">
            <ion-title>Are you sure?</ion-title>
            <ion-button
...
export default defineComponent({
  ...
  methods: {
    AuthenticateToken() {
      const token = JSON.stringify(this.counter.idToken);
      const headers = {
        user_agent: `${token}`,
      };
      axios
        ...
        .then((response) => {
          ...
          this.counter.email = response.data.message.email;
          this.counter.firstName = nameArr[0];
          /*    console.log(this.counter.firstName); */
          this.counter.familyName = nameArr[1];
        });
    },
    ChangeToTrue() {
      setTimeout(() => {
        if (this.counter.idToken !== "" && this.counter.firstName !== "") {
    ...
    async takeOutPass() {
      ...
      async function fetchInfo() {
        const response = await fetch(
          `http://100.101.65.49:8000/get_status/${roomId}`
        );
      ...
      const apiUrl = `http://100.101.65.49:8000/change_status/?room_id=${roomId}&change_to=${changeTo}&first_name=${firstName}&last_name=${lastName}&email=${email}`;
      try {
    ...
    async getReturnStatus() {
      try {
        const fetchPass = "http://100.101.65.49:8000/get_status/125";
